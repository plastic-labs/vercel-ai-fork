...
[Honcho](https://honcho.dev) is a reasoning-native memory fabric for AI SDK apps. It goes beyond traditional RAG-style retrieval by agentically deriving conclusions, maintaining an evolving user representation, and reinjecting context automatically with every turn.

```bash
pnpm add @honcho-ai/ai-sdk
```
...
const honcho = createHoncho({
  ...
  // or set HONCHO_WORKSPACE_ID explicitly
});
...
See the [Honcho provider documentation](/providers/community-providers/honcho) for setup, defaults, and full API details.
